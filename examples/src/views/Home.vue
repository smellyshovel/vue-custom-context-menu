<template>
<div>
    <h1>Examples index</h1>

    <ul>
        <li><a href="/directives-behavior">Directive's behavior</a></li>
        <li><a href="/context-menu-items-action">Context menu item's action</a></li>
        <li><a href="/actions-arguments">Action's arguments</a></li>
        <li><a href="/nested-opening-and-closing">Nested opening and closing</a></li>
        <li><a href="/penetrable-option">Penetrable option</a></li>
        <li><a href="/shift-option-and-positioning">Shift option and positioning</a></li>
        <li><a href="/wrapped-declarations">Wrapped declarations</a></li>
        <li><a href="/callbacks">Callbacks</a></li>
        <li><a href="/directive-dynamics">Directive dynamics</a></li>
    </ul>
</div>
</template>
